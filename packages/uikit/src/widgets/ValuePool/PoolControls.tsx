...
export function PoolControls<T>({
  ...
  const [finishedPools, openPools] = [[],pool?.tokens ?? []];
  const openPoolsWithStartBlockFilter = useMemo(
    () => openPools.filter((p: any) => (threshHold > 0 && p?.startBlock ? Number(p?.startBlock) < threshHold : true)),
    [threshHold, openPools]
  );
  const stakedOnlyFinishedPools = useMemo(
    () =>
      finishedPools?.filter((p: any) => {
        return p?.userData && new BigNumber(p?.userData?.stakedBalance).isGreaterThan(0);
      }),
    [finishedPools]
  );
  const stakedOnlyOpenPools = useCallback(() => {
    return openPoolsWithStartBlockFilter?.filter((p: any) => {
      return p?.userData && new BigNumber(p?.userData?.stakedBalance).isGreaterThan(0);
    });
  ...
  chosenPools = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      return sortedPools?.filter((p: any) =>
        latinise(p?.owner?.symbol?.toLowerCase() || "").includes(lowercaseQuery)
      );
    }
    return sortedPools;
  }, [
      ...
      <PoolControlsView>
        ...
        <FilterContainer>
          ...
          <LabelWrapper style={{ marginLeft: 16 }}>
            ...
            <SearchInput initialValue={searchQuery} onChange={handleChangeSearchQuery} placeholder={t("Search Pools")} />
          </LabelWrapper>
        </FilterContainer>
      </PoolControlsView>
