...
const PoolTabButtons = ({
  ...
  const isExact = router.pathname.includes('cards/bounties/admin')
  ...
  const liveOrFinishedSwitch = (
    <Wrapper>
      <ButtonMenu activeIndex={isExact ? 1 : 0} scale="sm" variant="subtle">
        <ButtonMenuItem as={NextLinkFromReactRouter} to="/cards" replace>
          {t('Cards')}
        </ButtonMenuItem>
        <ButtonMenuItem as={NextLinkFromReactRouter} to="/cards/bounties/admin" replace>
          {t('Bounties')}
        </ButtonMenuItem>
      </ButtonMenu>
    </Wrapper>
