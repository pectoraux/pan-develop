...
const PoolTabButtons = ({
  ...
  const isExact = router.pathname.includes('arps/bounties/admin')
  ...
  const liveOrFinishedSwitch = (
    <Wrapper>
      <ButtonMenu activeIndex={isExact ? 1 : 0} scale="sm" variant="subtle">
        <ButtonMenuItem as={NextLinkFromReactRouter} to="/arps" replace>
          {t('ARPs')}
        </ButtonMenuItem>
        <ButtonMenuItem as={NextLinkFromReactRouter} to="/arps/bounties/admin" replace>
          {t('Bounties')}
        </ButtonMenuItem>
      </ButtonMenu>
    </Wrapper>
