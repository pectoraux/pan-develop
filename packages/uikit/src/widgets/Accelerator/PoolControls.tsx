...
export function PoolControls<T>({
  ...
  console.log("watchlistTokens===========>", watchlistTokens, pools)
  chosenPools = useMemo(() => {
    const sortedPools = sortPools<T>(account, sortOption, chosenPools).slice(0, numberOfPoolsVisible)
    .filter((p: any) => favoritesOnly ? watchlistTokens.includes(`accelerator-${p.id}`) : true)
      ...
      <PoolControlsView>
        ...
        <FilterContainer>
          ...
          <LabelWrapper style={{ marginLeft: 16 }}>
            ...
            <SearchInput initialValue={searchQuery} onChange={handleChangeSearchQuery} placeholder={t("Search Pitches")} />
          </LabelWrapper>
        </FilterContainer>
      </PoolControlsView>
