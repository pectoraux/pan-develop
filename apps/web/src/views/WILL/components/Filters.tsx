...
const Filters: React.FC<any> = ({ nftFilters, setNewFilters, country=false, city=false, product=false }) => {
  ...
  const workspaceItems: Item[] = workspaces.Workspace?.map((attr: any) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const countryItems: Item[] = countries.Country?.map((attr: any) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const cityItems: Item[] = cities.City?.map((attr: any) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const productItems: Item[] = productsHome.Product?.map((attr: any) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
