...
const CreateWILLModal: React.FC<any> = ({ onDismiss }) => {
  ...
  const handleCreateGauge = useCallback(async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
      return callWithGasPrice(willFactoryContract, 'createGauge', [account, name, avatar, description])
      .catch((err) => {
        setPendingFb(false);
        toastError(
    ...
    if (receipt?.status) {
      ...
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('You can now start processing transactions through your Will pool.')}
        </ToastDescriptionWithTx>,
      )
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          <Text small color="textSubtle">
            {t('The will create a new Will Pool with you as its Admin. Please read the documentation to learn more about Will Pools.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
