...
import { LockStage,  WILLState } from './types'
...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    name: pool?.name,
    applicationLink: pool?.will?.applicationLink,
    willDescription: pool?.will?.willDescription,
    contactChannels: [],
    contacts: [],
    workspaces: [],
    countries: [],
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onApproveSuccess: async ({ receipt }) => {
      toastSuccess(
        t('Contract approved - you can now transfer tokens into this WILL!'),
    ...
    onConfirm: () => {
      if (stage === LockStage.CONFIRM_UPDATE_PROTOCOL) {
        ...
        return callWithGasPrice(willContract, 'updateProtocol', [
          account,
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        return callWithGasPrice(willContract, 'updateParameters', [
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        return callWithGasPrice(willContract, 'autoCharge', [state.accounts, 0, 1])
        .catch((err) => console.log("CONFIRM_ADMIN_AUTOCHARGE===============>", err, willContract))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        return callWithGasPrice(willHelperContract, 'emitUpdateWillDescription', [
          pool.willAddress, 
          state.willDescription, 
          state.avatar, 
      ...
      if (stage === LockStage.CONFIRM_WITHDRAW) {
        ...
        return callWithGasPrice(willContract, 'withdraw', [
          currency.address, 
          amount.toString()
        ])
      ...
      if (stage === LockStage.CONFIRM_DELETE_PROTOCOL) {
        return callWithGasPrice(willContract, 'deleteProtocol', [currAccount])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_DELETE) {
        return callWithGasPrice(willHelperContract, 'deleteWill', [pool?.willAddress])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_BOUNTY_ID) {
        return callWithGasPrice(willContract, 'updateBounty', [state.bountyId])
          .catch((err) => console.log("CONFIRM_UPDATE_BOUNTY===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_OWNER) {
        return callWithGasPrice(willContract, 'updateOwner', [account, state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_OWNER===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_TOKEN_ID) {
        return callWithGasPrice(willContract, 'updateTokenId', [state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_TOKEN_ID===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_AUTOCHARGE) {
        return callWithGasPrice(willContract, 'updateAutoCharge', [!!state.autoCharge])
        .catch((err) => console.log("CONFIRM_UPDATE_AUTOCHARGE===============>", err))
      }
      if (stage === LockStage.CONFIRM_VOTE) {
        return callWithGasPrice(willHelperContract, 'vote', [
          pool.willAddress,
          state.profileId,
          !!state.like
        ])
