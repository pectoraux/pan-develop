...
const Staked: React.FunctionComponent<any> = ({ pool, currAccount }) => {
  ...
  const willCurrencyInput = useCurrency(currencyId)
  const [currency, setCurrency] = useState(willCurrencyInput)
  ...
  return (
    <ActionContainer>
      ...
      <CurrencyInputPanel
        showInput={false}
        currency={currency ?? willCurrencyInput}
        onCurrencySelect={handleInputSelect}
        otherCurrency={currency ?? willCurrencyInput}
        id={pool?.sousId}
      />
      <ActionContent>
        <Button
