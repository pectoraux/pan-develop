...
const PoolStatsInfo: React.FC<any> = ({
  ...
  const earningToken = useMemo(() => pool?.tokens?.find((n) => n.tokenAddress === currState2[pool?.id]), [pool, currState2]) 
  ...
  return (
    <Flex flexDirection='column' maxHeight='200px' overflow='auto'>
      <Box><ReactMarkdown>{currAccount?.description}</ReactMarkdown></Box>
      <Flex flex="1" flexDirection="column" alignSelf="flex-center">
      {pool?.tokens?.length ? pool?.tokens.map((token) => (
        <Button
          key={token.id}
          onClick={() => {
            const newState = { ...currState2, [pool?.id]: token.tokenAddress}
            dispatch(setCurrBribeData(newState))
          }}
          mt="4px"
          mr={['2px', '2px', '4px', '4px']}
          scale="sm"
          variant={currState2[pool?.id] === token.tokenAddress ? 'subtle' : 'tertiary'}
        >
          {token?.symbol}
        </Button>
        ))
      ...
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
        <LinkExternal href={`/info/token/${pool?.willAddress}`} bold={false} small>
          {t('See Will Contract')}
        </LinkExternal>
      </Flex>
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
      ...
      {account && tokenAddress && (
        <Flex justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <AddToWalletButton
            ...
            tokenDecimals={pool?.earningToken?.decimals}
