...
import { useERC20, useAuditorContract, useAuditorHelper } from 'hooks/useContract'
...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  ...
  console.log("mcurrencyy===============>", currency, pool, auditorContract)
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    name: pool?.name,
    applicationLink: pool?.auditor?.applicationLink,
    auditorDescription: pool?.auditor?.auditorDescription,
    contactChannels: [],
    contacts: [],
    workspaces: [],
    countries: [],
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onConfirm: () => {
      if (stage === LockStage.CONFIRM_UPDATE_PROTOCOL) {
        ...
        return callWithGasPrice(auditorContract, 'updateProtocol', [
          account,
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        return callWithGasPrice(auditorContract, 'updateParameters', [
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        return callWithGasPrice(auditorContract, 'autoCharge', [state.accounts, 0, 1])
        .catch((err) => console.log("CONFIRM_ADMIN_AUTOCHARGE===============>", err, auditorContract))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        return callWithGasPrice(auditorHelperContract, 'emitUpdateAuditorDescription', [
          pool.auditorAddress, 
      ...
      if (stage === LockStage.CONFIRM_DELETE_PROTOCOL) {
        return callWithGasPrice(auditorContract, 'deleteProtocol', [currAccount])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_DELETE) {
        return callWithGasPrice(auditorHelperContract, 'deleteAuditor', [pool?.auditorAddress])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_BOUNTY_ID) {
        return callWithGasPrice(auditorContract, 'updateBounty', [state.bountyId])
          .catch((err) => console.log("CONFIRM_UPDATE_BOUNTY===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_OWNER) {
        return callWithGasPrice(auditorContract, 'updateOwner', [account, state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_OWNER===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_TOKEN_ID) {
        return callWithGasPrice(auditorContract, 'updateTokenId', [state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_TOKEN_ID===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_AUTOCHARGE) {
        return callWithGasPrice(auditorContract, 'updateAutoCharge', [!!state.autoCharge])
        .catch((err) => console.log("CONFIRM_UPDATE_AUTOCHARGE===============>", err))
      }
      if (stage === LockStage.CONFIRM_VOTE) {
        return callWithGasPrice(auditorHelperContract, 'vote', [
          pool.auditorAddress,
          state.profileId,
          !!state.like
        ])
