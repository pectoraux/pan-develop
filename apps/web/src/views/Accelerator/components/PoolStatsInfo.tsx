...
import { setCurrBribeData } from 'state/businesses'
import { useCurrBribe } from 'state/businesses/hooks'
...
const PoolStatsInfo: React.FC<any> = ({
  ...
  return (
    <Flex flexDirection='column' maxHeight='200px' overflow='auto'>
      ...
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
        <LinkExternal href={`https://${SCAN_DOMAIN[chainId]}.com/address/${vestingTokenAddress}`} bold={false} small>
          {t('See Token Info')}
        </LinkExternal>
      </Flex>
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
