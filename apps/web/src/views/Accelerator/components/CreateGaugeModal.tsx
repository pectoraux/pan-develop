...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  const [stage, setStage] = useState<any>(variant==="admin" 
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onConfirm: async () => {
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        try {
          return callWithGasPrice(acceleratorVoterContract, 'updateContent', [
            ["", "", "", original, thumbnail],
            title,
            description
          ])
        } catch (err2) {
          return console.log("CONFIRM_UPDATE_PARAMETERS==================>", err2)
        }
      }
