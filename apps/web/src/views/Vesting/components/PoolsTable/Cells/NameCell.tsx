...
const NameCell: React.FC<any> = ({ pool, vpCurrencyInput }) => {
  ...
  const stakingTokenSymbol = vpCurrencyInput?.symbol ?? ''

  const title: React.ReactNode = pool?.name
  const subtitle1: React.ReactNode = `${t('Save')} ${stakingTokenSymbol}`
  const subtitle2: React.ReactNode = `${t('Receive')} ${pool.vaSymbol}`
