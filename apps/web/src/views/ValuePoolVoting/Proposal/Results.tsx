...
const Results: React.FC<any> = ({ proposal, hasAccountVoted }) => {
  ...
  const totalVotes = proposal.votes.reduce((ac, v) => ac + parseInt(v.votingPower), 0)
  const total1Votes = proposal.votes.filter((v) => v.choice === "Attacker")
  .reduce((ac, v) => ac + parseInt(v.votingPower), 0)
  const total2Votes = proposal.votes.filter((v) => v.choice === "Defender")
  .reduce((ac, v) => ac + parseInt(v.votingPower), 0)
  const count1 = proposal.votes.filter((v) => v.choice === "Attacker")?.length
  const count2 = proposal.votes.filter((v) => v.choice === "Defender")?.length
  const progress1 = total1Votes * 100 / Math.max(totalVotes, 1)
  const progress2 = total2Votes * 100 / Math.max(totalVotes, 1)
  const afterOneWeek = convertTimeToSeconds(proposal.endTime) < Date.now()
  ...
  const handleApplyResults = useCallback(async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
        console.log("stakeMarketVoterContract====================>", [
          proposal.ve,
          proposal.id,
          proposal.attackerId
        ])
      ...
      <CardBody>
        ...
        <Box key='defender' mt='24px'>
          ...
          {afterOneWeek ?
            ...
            <Countdown
              nextEventTime={convertTimeToSeconds(proposal.endTime)}
