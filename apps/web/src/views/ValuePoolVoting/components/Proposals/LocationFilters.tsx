...
const Filters: React.FC<any> = () => {
  ...
  const paywallsHome = Object.entries(PAYWALLS)?.filter((item: any) => nftFilters.workspace ? item[1].Workspace === nftFilters.workspace?.value : true)
  .reduce((accum: any, attr: any) => ({
      ...accum,
      Paywall: nftFilters.city ? 
      (accum.Paywall ? [...accum.Paywall, {traitType: "Paywall", value: attr[0], count: attr[1].City[nftFilters.city.value]}]:
      [{traitType: "Paywall", value: attr[0], count: nftFilters?.length ? attr[1].City[nftFilters.city.value] : 0 }]):
      nftFilters.country ? 
      (accum.Paywall ? [...accum.Paywall, {traitType: "Paywall", value: attr[0], count: attr[1].Country[nftFilters.country.value]}]:
      [{traitType: "Paywall", value: attr[0], count: nftFilters?.length ? attr[1].Country[nftFilters.country.value] : 0 }]): 
      (accum.Paywall ? [...accum.Paywall, {traitType: "Paywall", value: attr[0], count: attr[1].Total}]:
