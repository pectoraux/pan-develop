...
const Pools: React.FC<any> = () => {
  ...
  const { pools, userDataLoaded: sponsorsLoaded } = usePoolsWithFilterSelector()
  ...
  const userDataLoaded = isBounties ? bountiesLoaded : sponsorsLoaded
        ...
        <Flex justifyContent="space-between" flexDirection={['column', null, null, 'row']}>
          <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
            <Heading as="h1" scale="xxl" color="secondary" mb="24px">
              {isBounties ? t('Trust Bounties') : t('Sponsor Cards')}
            </Heading>
            <Heading scale="md" color="text">
              {isBounties ? t('Create a trust bounty to improve trust with your customers.') : t('Browse through all sponsors...')}
            </Heading>
            <Heading scale="md" color="text">
              {isBounties ? null : t('And apply to work with your favorites')}
            </Heading>
            <Flex>
              <Button p="0" variant="text">
                <Text color="primary" onClick={isBounties ? onPresentTrustBounties : onPresentCreateGauge} bold fontSize="16px" mr="4px">
                  {isBounties ? t('Create an Bounty in ') : t('Create a sponsor card')}
                </Text>
                {isBounties &&
                <CurrencyInputPanel
                  showInput={false}
