...
const PreviousRoundCardFooter: React.FC<any> = ({ lotteryNodeData, lotteryId, currentTokenId }) => {
  ...
  const currTokenData = useMemo(() => lotteryNodeData.tokenData?.length ? lotteryNodeData.tokenData[parseInt(currentTokenId)] : {}, [lotteryNodeData, currentTokenId])
      ...
      <>
        {prizeInBusd.isNaN() ? (
          ...
          <Heading scale="xl" lineHeight="1" color="secondary">
            {formatNumber(getBalanceNumber(prizeInBusd), 0, 0)} {` ${currTokenData?.token?.symbol?.toUpperCase() ?? ''}`}
          </Heading>
        )}
        {/* {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            value={getBalanceNumber(lotteryNodeData?.priceTicket)}
            decimals={0}
          />
        )} */}
      </>
