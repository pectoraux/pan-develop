...
import { usePoolsPageFetch, usePoolsWithFilterSelector } from 'state/card/hooks'
...
import CreateGaugeModal from 'views/Cards/components/CreateGaugeModal'
...
const Pools: React.FC<any> = () => {
  ...
  const { card } = router.query as any
  const { pools, userDataLoaded } = usePoolsWithFilterSelector()
  const ogCard = pools?.find((pool) => pool?.id?.toLowerCase() === card?.toLowerCase())
  const inputCurency = useCurrency(DEFAULT_TFIAT ?? undefined)
  const [currency, setCurrency] = useState(inputCurency)
  const [onPresentAdminSettings] = useModal(<CreateGaugeModal variant='admin' location='fromCard' currency={currency} pool={ogCard} />,)
        ...
        <Flex justifyContent="space-between" flexDirection={['column', null, null, 'row']}>
          <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
            ...
            <Heading scale="md" color="text">
              {t(ogCard?.collection?.description ?? '')}
            </Heading>
          {ogCard?.devaddr_?.toLowerCase() === account?.toLowerCase() ? 
            <Flex>
          ...
          {({ chosenPools, viewMode, stakedOnly, normalizedUrlSearch }) => (
            <>
              ...
              {viewMode === ViewMode.CARD ? (
                ...
                <PoolsTable urlSearch={normalizedUrlSearch} ogCard={ogCard} pools={chosenPools} account={account} />
              )}
              <Image
                mx="auto"
                mt="12px"
