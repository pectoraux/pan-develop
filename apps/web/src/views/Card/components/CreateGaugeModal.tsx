...
import { useERC20, useCardContract, useCardHelper } from 'hooks/useContract'
...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  ...
  const cardContract = useCardContract(pool?.cardAddress || router.query.card || '')
  const cardHelperContract = useCardHelper()
  const [nftFilters, setNewFilters] = useState({} as any)
  const currAccount = pool.accounts?.find((acct) => acct === account)
  console.log("mcurrencyy===============>", currency, pool, cardContract)
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    name: pool?.name,
    applicationLink: pool?.card?.applicationLink,
    cardDescription: pool?.card?.cardDescription,
    contactChannels: [],
    contacts: [],
    workspaces: [],
    countries: [],
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onConfirm: () => {
      if (stage === LockStage.CONFIRM_UPDATE_PROTOCOL) {
        ...
        return callWithGasPrice(cardContract, 'updateProtocol', [
          account,
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        return callWithGasPrice(cardContract, 'updateParameters', [
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        return callWithGasPrice(cardContract, 'autoCharge', [state.accounts, 0, 1])
        .catch((err) => console.log("CONFIRM_ADMIN_AUTOCHARGE===============>", err, cardContract))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        return callWithGasPrice(cardHelperContract, 'emitUpdateCardDescription', [
          pool.cardAddress, 
      ...
      if (stage === LockStage.CONFIRM_WITHDRAW) {
        ...
        return callWithGasPrice(cardContract, 'withdraw', [
          currency.address, 
          amount.toString()
        ])
      ...
      if (stage === LockStage.CONFIRM_DELETE_PROTOCOL) {
        return callWithGasPrice(cardContract, 'deleteProtocol', [currAccount])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_DELETE) {
        return callWithGasPrice(cardHelperContract, 'deleteCard', [pool?.cardAddress])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_BOUNTY_ID) {
        return callWithGasPrice(cardContract, 'updateBounty', [state.bountyId])
          .catch((err) => console.log("CONFIRM_UPDATE_BOUNTY===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_OWNER) {
        return callWithGasPrice(cardContract, 'updateOwner', [account, state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_OWNER===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_TOKEN_ID) {
        return callWithGasPrice(cardContract, 'updateTokenId', [state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_TOKEN_ID===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_AUTOCHARGE) {
        return callWithGasPrice(cardContract, 'updateAutoCharge', [!!state.autoCharge])
        .catch((err) => console.log("CONFIRM_UPDATE_AUTOCHARGE===============>", err))
      }
      if (stage === LockStage.CONFIRM_VOTE) {
        return callWithGasPrice(cardHelperContract, 'vote', [
          pool.cardAddress,
          state.profileId,
          !!state.like
        ])
      ...
      {stage === LockStage.SETTINGS && 
        <Flex flexDirection="column" width="100%" px="16px" pt="16px" pb="16px">
          <Flex justifyContent='center' alignSelf='center' mb='10px'>
            <LinkExternal color='success' href={pool.card?.applicationLink} bold>
