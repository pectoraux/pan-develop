      ...
      <Flex flexDirection='column'>
        <Flex flexDirection='row'>
          ...
          <Balance
            ...
            color={Number(pool.card?.likes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.card?.likes) ? pool.card?.likes : 0}
          />
        </Flex>
        <Flex flexDirection='row' position='relative' bottom='5px'>
          ...
          <Balance
            bold={!isMobile}
            fontSize={isMobile ? '14px' : '16px'}
            color={Number(pool.card?.dislikes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.card?.dislikes) ? pool.card?.dislikes : 0}
          />
        </Flex>
      </Flex>
    </StyledCell>
