...
const CreateAuditorModal: React.FC<any> = ({ onDismiss }) => {
  ...
  const handleCreateGauge = useCallback(async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
      ...
      try {
      return callWithGasPrice(worldFactoryContract, 'createGauge', [profileId, account])
      .catch((err) => {
        toastError(
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          <Text small color="textSubtle">
            {t('The will create a new World contract with you as its Admin. Please read the documentation to learn more about Auditor Pools.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
