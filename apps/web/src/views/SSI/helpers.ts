...
import { Proposal, EntryState, EntryType, Vote, Entry } from 'state/types'
...
export const filterProposalsByState = (proposals: any, state: any) => {
  if (state === EntryState.ACTIVE) {
    ...
    convertTimeToSeconds(proposal.startTime) <= Date.now() &&
    Date.now() <= convertTimeToSeconds(proposal.endTime)) 
  }
