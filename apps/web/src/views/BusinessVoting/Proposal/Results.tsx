...
const Results: React.FC<any> = ({ business, accountVote, hasAccountVoted }) => {
  ...
  const totalVotes = business.votes.reduce((ac, v) => ac + parseInt(v.votingPower), 0)
      ...
      <CardBody>
        <Box key='voted' mt='24px'>
          ...
          <Flex alignItems="center" justifyContent="space-between">
            <Text color="textSubtle">{t('%total% Vote(s)', { total: formatNumber(business.votes.length, 0, 2) })}</Text>
            <Text>
