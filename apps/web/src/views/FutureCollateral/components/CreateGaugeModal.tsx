...
import { useERC20, useFutureCollateralContract, useFutureCollateralNote, useFutureCollateralHelper, useFutureCollateralHelper2 } from 'hooks/useContract'
...
const BuyModal: React.FC<any> = ({ variant="user", location='fromStake', pool, currAccount, currency, onDismiss }) => {
  ...
  const futureCollateralContract = useFutureCollateralContract(pool?.futureCollateralAddress || router.query.futureCollateral || '')
  const futureCollateralNoteContract = useFutureCollateralNote()
  const futureCollateralHelperContract = useFutureCollateralHelper()
  const futureCollateralHelper2Contract = useFutureCollateralHelper2()
  console.log("mcurrencyy===============>", currAccount, currency, pool, futureCollateralContract)
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    pricePerMinute: '',
    contractAddress: '',
    futureCollateral: pool?.futureCollateralAddress ?? '',
    ...
    futureCollateralDescription: pool?.futureCollateralDescription ?? '',
    datakeeper: 0,
    accounts: [],
  }))
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    onRequiresApproval: async () => {
      try {
        return requiresApproval(stakingTokenContract, account, futureCollateralContract.address)
      } catch (error) {
        return true
      }
    },
    onApprove: () => {
      return callWithGasPrice(stakingTokenContract, 'approve', [futureCollateralContract.address, MaxUint256])
    },
    ...
    onConfirm: () => {
      if (stage === LockStage.CONFIRM_UPDATE_SPONSOR_MEDIA) {
        ...
        return callWithGasPrice(futureCollateralHelper2Contract, 'updateSponsorMedia', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_TAG_REGISTRATION) {
        ...
        return callWithGasPrice(futureCollateralHelper2Contract, 'updateTagRegistration', args)
        .catch((err) => console.log("CONFIRM_UPDATE_TAG_REGISTRATION===============>", err))
      }
      if (stage === LockStage.CONFIRM_TRANSFER_TO_NOTE_RECEIVABLE) {
        const args = [state.futureCollateral,state.toAddress,state.protocolId,state.numPeriods]
        console.log("CONFIRM_TRANSFER_TO_NOTE_RECEIVABLE===============>", args)
        return callWithGasPrice(futureCollateralNoteContract, 'transferDueToNoteReceivable', args)
        .catch((err) => console.log("CONFIRM_TRANSFER_TO_NOTE_RECEIVABLE===============>", err))
      }
      if (stage === LockStage.CONFIRM_TRANSFER_TO_NOTE_PAYABLE) {
        const args = [state.futureCollateral,state.toAddress,state.protocolId,state.amountPayable]
        console.log("CONFIRM_TRANSFER_TO_NOTE_PAYABLE===============>", args)
        return callWithGasPrice(futureCollateralNoteContract, 'transferDueToNotePayable', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PROTOCOL) {
        ...
        console.log("CONFIRM_UPDATE_PROTOCOL===============>", futureCollateralContract, args)
        return callWithGasPrice(futureCollateralContract, 'updateProtocol', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateParameters', args)
      ...
      if (stage === LockStage.CONFIRM_SPONSOR_TAG) {
        ...
        const args = [
          ...
          state.futureCollateral, 
          amountReceivable.toString(), 
          state.tag,
          state.message
        ]
        console.log("CONFIRM_SPONSOR_TAG===============>",args)
        return callWithGasPrice(futureCollateralHelper2Contract, 'sponsorTag', args)
      ...
      if (stage === LockStage.CONFIRM_MINT_EXTRA) {
        ...
        return callWithGasPrice(futureCollateralHelperContract, 'mintExtra', args)
        .catch((err) => console.log("CONFIRM_MINT_EXTRA===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DATA_KEEPER) {
        const args = [state.futureCollateral, !!state.datakeeper]
        console.log("CONFIRM_UPDATE_DATA_KEEPER===============>",args)
        return callWithGasPrice(futureCollateralNoteContract, 'updateDatakeeper', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_EXCLUDED_CONTENT) {
        ...
        return callWithGasPrice(futureCollateralHelper2Contract, 'updateExcludedContent', [state.tag, state.contentType, !!state.add])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PRICE_PER_MINUTE) {
        ...
        return callWithGasPrice(futureCollateralHelper2Contract, 'updatePricePerAttachMinutes', [pricePerMinute?.toString()])
      ...
      if (stage === LockStage.CONFIRM_BURN) {
        ...
        return callWithGasPrice(futureCollateralHelperContract, 'burn', [state.tokenId])
        .catch((err) => console.log("CONFIRM_BURN===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_CATEGORY) {
        console.log("CONFIRM_UPDATE_CATEGORY===============>",[state.futureCollateral, state.category])
        return callWithGasPrice(futureCollateralHelperContract, 'updateCategory', [state.futureCollateral, state.category])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_MINT_INFO) {
        ...
        return callWithGasPrice(futureCollateralHelperContract, 'updateMintInfo', [
          state.futureCollateral, 
          state.extraMint, 
          state.tokenId
        ])
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        ...
        return callWithGasPrice(futureCollateralContract, 'autoCharge', [state.accounts?.split(','), state.numPeriods])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        console.log("CONFIRM_UPDATE_DESCRIPTION===============>", [
          state.futureCollateral, 
          state.futureCollateralDescription, 
          state.avatar, 
        ...
        return callWithGasPrice(futureCollateralNoteContract, 'emitUpdateFutureCollateralDescription', [
          state.futureCollateral, 
          state.futureCollateralDescription, 
          state.avatar, 
          state.applicationLink, 
      ...
      if (stage === LockStage.CONFIRM_WITHDRAW) {
        ...
        return callWithGasPrice(futureCollateralContract, 'withdraw', [
          currency?.address, 
          amount.toString()
        ])
      ...
      if (stage === LockStage.CONFIRM_DELETE_PROTOCOL) {
        ...
        return callWithGasPrice(futureCollateralContract, 'deleteProtocol', [state.protocolId])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_DELETE) {
        console.log("CONFIRM_DELETE_PROTOCOL===============>",[state.futureCollateral])
        return callWithGasPrice(futureCollateralNoteContract, 'deleteFutureCollateral', [state.futureCollateral])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_BOUNTY_ID) {
        console.log("CONFIRM_UPDATE_BOUNTY===============>", [state.bountyId, state.protocolId])
        return callWithGasPrice(futureCollateralContract, 'updateBounty', [state.bountyId, state.protocolId])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_AUTOCHARGE) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateAutoCharge', [!!state.autoCharge, state.protocolId])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_URI_GENERATOR) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateURIGenerator', [state.uriGenerator])
        .catch((err) => console.log("CONFIRM_UPDATE_URI_GENERATOR===============>", err))
      }
      if (stage === LockStage.CONFIRM_CLAIM_NOTE) {
        console.log("CONFIRM_CLAIM_NOTE===============>",[state.tokenId])
        return callWithGasPrice(futureCollateralNoteContract, 'claimPendingRevenueFromNote', [state.tokenId])
        .catch((err) => console.log("CONFIRM_CLAIM_NOTE===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_RATING_LEGEND) {
        const args = [state.futureCollateral, state.legend?.split(',')]
        console.log("CONFIRM_UPDATE_RATING_LEGEND===============>",args)
        return callWithGasPrice(futureCollateralHelper2Contract, 'updateRatingLegend', args)
      ...
      if (stage === LockStage.CONFIRM_CLAIM_REVENUE_FROM_SPONSORS) {
        ...
        return callWithGasPrice(futureCollateralHelper2Contract, 'claimPendingRevenue', [])
      ...
      if (stage === LockStage.CONFIRM_VOTE) {
        ...
        return callWithGasPrice(futureCollateralNoteContract, 'vote', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_DISCOUNT_DIVISOR) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateDiscountDivisor', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PENALTY_DIVISOR) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updatePenaltyDivisor', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_ADMIN) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateAdmin', args)
      ...
      if (stage === LockStage.CONFIRM_UPDATE_OWNER) {
        ...
        return callWithGasPrice(futureCollateralContract, 'updateDev', args)
      ...
      {stage === LockStage.SETTINGS && 
        <Flex flexDirection="column" width="100%" px="16px" pt="16px" pb="16px">
          <Flex justifyContent='center' style={{ cursor: "pointer"}} alignSelf='center' mb='10px'>
            <LinkExternal color='success' href={pool.futureCollateral?.applicationLink} bold>
