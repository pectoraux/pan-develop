...
const Filters: React.FC<{
  ...
  city?: boolean
  product?: boolean
}> = ({ nftFilters, setNewFilters, country=false, city=false, product=false }) => {
  const workspaces: any = Object.entries(WORKSPACES)?.reduce(
    (accum: any, attr) => ({
      ...accum,
  ...
  const countries: any = Object.entries(COUNTRIES)?.reduce(
    (accum: any, attr: any) => ({
      ...accum,
  ...
  const cities: any = Object.entries(CITIES)?.reduce(
    (accum: any, attr: any) => ({
      ...accum,
      City: accum.City 
  ...
  const productsHome: any = Object.entries(PRODUCTS2)?.reduce(
    (accum: any, attr: any) => ({
      ...accum,
  ...
  const workspaceItems = workspaces.Workspace.map((attr) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const countryItems = countries.Country.map((attr) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const cityItems = cities.City.map((attr) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
  const productItems = productsHome.Product?.map((attr) => ({
    label: attr.value as string,
    count: attr.count ? attr.count : undefined,
    attr,
  }))
