...
const CreateARPModal: React.FC<SetPriceStageProps> = ({ onDismiss }) => {
  ...
  const handleCreateGauge = useCallback(async () => {
      ...
      const receipt = await fetchWithCatchTxError(async () => {
        try {
          return callWithGasPrice(arpFactoryContract, 'createGauge', [account, name, avatar, description])
        } catch (err) {
          setPendingFb(false)
        //   toastError(
      ...
      if (receipt?.status) {
        ...
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('You can now start processing transactions through your Arp pool.')}
          </ToastDescriptionWithTx>,
        )
        dispatch(fetchArpsAsync({ fromArp: true }))
      }
      onDismiss()
  }, [t, name, avatar, account, dispatch, onDismiss, description, toastSuccess, callWithGasPrice, fetchWithCatchTxError, arpFactoryContract])
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          <Text small color="textSubtle">
            {t('The will create a new Arp Pool with you as its Admin. Please read the documentation to learn more about Arp Pools.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
