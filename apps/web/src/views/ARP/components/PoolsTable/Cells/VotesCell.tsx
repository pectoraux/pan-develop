      ...
      <Flex flexDirection='column'>
        <Flex flexDirection='row'>
          ...
          <Balance
            ...
            color={Number(pool.arp?.likes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.arp?.likes) ? pool.arp?.likes : 0}
          />
        </Flex>
        <Flex flexDirection='row' position='relative' bottom='5px'>
          ...
          <Balance
            bold={!isMobile}
            fontSize={isMobile ? '14px' : '16px'}
            color={Number(pool.arp?.dislikes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.arp?.dislikes) ? pool.arp?.dislikes : 0}
          />
        </Flex>
      </Flex>
    </StyledCell>
