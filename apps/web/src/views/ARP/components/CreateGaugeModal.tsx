...
import { useERC20, useARPContract, useARPHelper } from 'hooks/useContract'
...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  ...
  const [nftFilters, setNewFilters] = useState<any>()
  const currAccount = pool.accounts?.find((acct) => acct === account)
  console.log("mcurrencyy===============>", currency, pool, arpContract)
  // const [onPresentPreviousTx] = useModal(<ActivityHistory />,)

  // console.log("router===================>", router)
  const [state, setState] = useState<any>(() => ({
    owner: pool?.owner,
    avatar: pool?.arp?.avatar,
    ...
    name: pool?.name,
    applicationLink: pool?.arp?.applicationLink,
    arpDescription: pool?.arp?.arpDescription,
    contactChannels: [],
    contacts: [],
    workspaces: [],
    countries: [],
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onConfirm: () => {
        ...
        return callWithGasPrice(arpContract, 'updateProtocol', [
          account,
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        return callWithGasPrice(arpContract, 'updateParameters', [
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        return callWithGasPrice(arpContract, 'autoCharge', [state.accounts, 0, 1])
        .catch((err) => console.log("CONFIRM_ADMIN_AUTOCHARGE===============>", err, arpContract))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        return callWithGasPrice(arpHelperContract, 'emitUpdateArpDescription', [
          pool.arpAddress, 
          state.arpDescription, 
          state.avatar, 
      ...
      if (stage === LockStage.CONFIRM_WITHDRAW) {
        ...
        return callWithGasPrice(arpContract, 'withdraw', [
          currency.address, 
          amount.toString()
        ])
