...
const BribeField: React.FC<any> = ({
  ...
  const token = useToken(stakingSymbol === 'BNB' ? stakingSymbol : stakingAddress)
  ...
  const handleChangePercent = useCallback(
    (sliderPercent: number) => {
      if (sliderPercent > 0) {
        ...
        if (typeof setShare === 'function') setShare(sliderPercent * 100)
...
export default memo(BribeField)
