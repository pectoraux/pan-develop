...
import { BunnyPlaceholderIcon, Spinner, Input, LinkExternal, Link, AutoRenewIcon, Button, Flex, Grid, Text, useModal, Dots } from '@pancakeswap/uikit'
  ...
  return (
    <>
      ...
      <CollapsibleCard 
        ...
        title={`${currentPartner.partnerCollection.name} -> ${currentPartner.mirrors?.filter((mirror) => !!mirror.nft)?.length ?? 0} ${t('Result(s)')}`}
        mb="32px">
                  ...
                  gridTemplateColumns={['1fr', '1fr', 'repeat(2, 1fr)', 'repeat(3, 1fr)']} mb="64px">
                    ...
                    >
                      ...
                      <Flex mb="2px" justifyContent='flex-end'>
                        <LinkExternal href={`${nftsBaseUrl}/collections/${registration.partnerCollection.id}`} bold={false} small>
                          {t('See Channel')}
                        </LinkExternal>
                      </Flex>
                    </CollectionCard>
          ...
          return <Grid gridGap="16px" 
                    ...
                    >
                      ...
                      <Flex mb="2px" justifyContent='flex-start'>
                        ...
                        <LinkExternal href={`${nftsBaseUrl}/collections/${registration.userCollection?.id}`} bold={false} small>
                          {t('See Channel')}
                        </LinkExternal>
                      </Flex>
                    </CollectionCard>
  ...
  return isAdmin && paywall?.mirrors?.length > 0 ? (
    <Flex flexDirection="column">
      ...
      <Grid
        ...
        alignItems="start"
      >
          {paywall.mirrors.filter((mirror) => !!mirror.nft).map((mirror) => {
            const {nft} = mirror
            const currentAskPriceAsNumber = nft && parseFloat(nft?.currentAskPrice)
            return (
              <CollectibleLinkCard
