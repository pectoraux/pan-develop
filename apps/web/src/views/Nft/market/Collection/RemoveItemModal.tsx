...
const PartnerModal: React.FC<any> = ({
  ...
  const item = useMemo(() => collection?.nfts?.find((it) => it.tokenId?.toLowerCase() === state.productId?.toLowerCase()),[
    collection,
    state
  ])
  ...
  const handleRemoveItem = useCallback(async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
      ...
      try {
      img0 = item.images[0] ? nodeRSA.decryptStringWithRsaPrivateKey({ 
        text: item.images[0], 
        privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY
      }):"";
      img1 = item.images[1] ? nodeRSA.decryptStringWithRsaPrivateKey({ 
        text: item.images[1], 
        privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY
      }):"";
      img2 = item.images[2] ? nodeRSA.decryptStringWithRsaPrivateKey({ 
        text: item.images[2], 
        privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY
      }):"";
      img3 = item.images[3] ? nodeRSA.decryptStringWithRsaPrivateKey({ 
        text: item.images[3], 
        privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY
      }):"";
      img4 = item.images[4] ? nodeRSA.decryptStringWithRsaPrivateKey({ 
        text: item.images[4], 
        privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY
      }):"";
      ...
      } catch(err) { console.log("handleRemoveItem err============>", err) }
