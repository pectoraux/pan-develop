...
const SellModal: React.FC<any> = ({
  ...
  const options = nftToSell?.option_categories?.map((cat, index) => {
    return {
    "id": `${index}`,
    "category": `${cat}`,
  ...
  const { signer: ordersSigner } = useNftMarketOrdersContract()
  const { signer: tradesSigner } = useNftMarketTradesContract()
  const { signer: helpersSigner } = useNftMarketHelperContract()
    ...
    onConfirm: () => {
      ...
      if (stage === SellingStage.CONFIRM_ADD_LOCATION) {
        ...
        console.log("CONFIRM_ADD_LOCATION==============>", [
          ...
          [...nftFilters?.product, ...state.customTags.split(",")]?.filter((val) => !!val)?.reduce((accum, attr) => ([...accum, attr]),[],)
        ])
      ...
      if (stage === SellingStage.CONFIRM_UPDATE_BURN_FOR_CREDIT_TOKENS) {
        ...
        return callWithGasPrice(helpersSigner, 'updateBurnTokenForCredit', [burnForCreditToken, discountNumber])
