...
const ActivityHistory: React.FC<any> = ({ collection }) => {
  ...
  useEffect(() => {
    ...
    // }
  }, [dispatch, collectionId, nftActivityFiltersString, lastUpdated])
  ...
  return (
    <Box py="32px">
      <Container px={[0, null, '94px']}>
      {show && <MintExternalModal collection={collection} />}
      </Container>
            ...
            <Table>
              ...
              <tbody>
                {!isInitialized ? (
                  ...
                  activitiesSlice.filter((activity) => !!activity?.nft || activity?.paywall)
                  .map((activity) => {
