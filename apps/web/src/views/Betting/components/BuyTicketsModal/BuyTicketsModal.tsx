...
const BuyTicketsModal: React.FC<any> = ({ betting, onDismiss }) => {
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    ...
    onConfirm: () => {
        ...
        const ticketNumbers = ticketsForPurchase.filter((ticket: any) => !!ticket).map((ticket) => encodeAlphabet(ticket, ticketSize))
        const args = [
          betting.bettingId,
          account,
          AddressZero,
        ...
        .catch((err) => console.log("ticketsForPurchase==============>", err))
      },
    ...
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCake && getFullDisplayBalance(priceTicketInCake.times(ticketsToBuy || 0))} {betting?.token?.symbol ?? ''}
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
