...
// import { getBettingsV1Address } from 'utils/addressHelpers'
...
const ClaimCheck = () => {
  ...
  const bettingsV1Address = "" // useMemo(() => getBettingsV1Address(), [])

  const [onPresentCollectWinningsModal] = useModal(
    <CollectRoundWinningsModal
      bettingsAddress={bettingsV1Address}
      token={bscTokens.bnb}
      dispatch={dispatch}
      history={history}
  ...
  return (
    <StyledClaimCheck onClick={account ? handleClick : undefined}>
      <Box style={{ flex: 1 }}>
        <Text>{t('Showing history for Betting v0.2')}</Text>
