...
// import { fetchNodeHistory, markAsCollected } from 'state/bettings'
...
const CollectRoundWinningsModal: React.FC<any> = ({
  onDismiss,
  onSuccess,
  history,
  isLoadingHistory,
  ...
  const handleClick = async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
      return callWithGasPrice(bettingsContract, 'claim', [epochs])
    })
    if (receipt?.status) {
      // if (!isV1Claim) {
