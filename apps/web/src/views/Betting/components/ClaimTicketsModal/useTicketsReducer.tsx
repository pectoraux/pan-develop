...
import { parseRetrievedNumber } from '../../helpers2'
...
const getInitialState = ({
  ...
  const randomTickets = generateTicketNumbers(userCurrentTickets)
  ...
  switch (action.type) {
    case 'updateTicket': {
      ...
      const updatedTicket = {
        ...
        isComplete: newNumbers.join('').length > 0,
      }
...
export const useTicketsReducer = (
  ...
  const updateTicket = (ticketId: number, newNumbers: string[]) => {
    console.log("updateTicket==============>", newNumbers)
  ...
  const getTicketsForPurchase = () => {
    return state.tickets.filter((ticket) => !!ticket.numbers).map((ticket) => {
