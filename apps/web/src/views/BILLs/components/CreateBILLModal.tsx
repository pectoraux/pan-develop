...
const CreateBILLModal: React.FC<any> = ({ onDismiss }) => {
  ...
  const handleCreateGauge = useCallback(async () => {
    ...
    const receipt = await fetchWithCatchTxError(async () => {
      console.log("receipt================>", billFactoryContract, [
        profileId, 
        account,
        !!isPayable
      ])
      return callWithGasPrice(billFactoryContract, 'createGauge', [
        profileId, 
        account,
        !!isPayable,
      ])
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          <Text small color="textSubtle">
            {t('The will create a new BILL contract with you as its Admin. Please read the documentation to learn more about BILLs.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
