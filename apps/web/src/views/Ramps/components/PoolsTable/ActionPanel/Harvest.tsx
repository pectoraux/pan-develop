      ...
      <ActionContent>
        <Flex flex="1" flexDirection="column" alignSelf="flex-center">
          ...
          <Box mr="8px" height="32px">
            <Balance lineHeight="1" color="textSubtle" fontSize="12px" decimals={rampAccount?.token?.decimals ?? 18} value={getBalanceNumber(rampAccount?.minted)} />
            <Text color="primary" fontSize="12px" display="inline" bold as="span" textTransform="uppercase">
              {t("Token Minted")}
            </Text>
          </Box>
          <Box mr="8px" height="32px">
            <Balance lineHeight="1" color="textSubtle" fontSize="12px" decimals={rampAccount?.token?.decimals ?? 18} value={getBalanceNumber(rampAccount?.burnt)} />
            <Text color="primary" fontSize="12px" display="inline" bold as="span" textTransform="uppercase">
              {t("Token Burnt")}
            </Text>
          </Box>
          <Box mr="8px" height="32px">
            <Balance lineHeight="1" color="textSubtle" fontSize="12px" decimals={rampAccount?.token?.decimals ?? 18} value={getBalanceNumber(rampAccount?.salePrice)} />
            <Text color="primary" fontSize="12px" display="inline" bold as="span" textTransform="uppercase">
              {t("Sale Price")}
            </Text>
          </Box>
