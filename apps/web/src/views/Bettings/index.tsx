...
const Pools: React.FC<any> = () => {
  ...
  const { pools, userDataLoaded: bettingsLoaded } = usePoolsWithFilterSelector()
  ...
  const userDataLoaded = isBounties ? bountiesLoaded : bettingsLoaded
  const inputCurency = useCurrency(DEFAULT_TFIAT)
  const [currency, setCurrency] = useState(inputCurency)
  const handleInputSelect = useCallback((currencyInput) => setCurrency(currencyInput),[],)
  const [onPresentCreateGauge] = useModal(<CreateBettingModal currency={currency ?? inputCurency} />,)
        ...
        <Flex justifyContent="space-between" flexDirection={['column', null, null, 'row']}>
          <Flex flex="1" flexDirection="column" mr={['8px', 0]}>
            <Heading as="h1" scale="xxl" color="secondary" mb="24px">
              {isBounties ? t('Trust Bounties') : t('Bettings Pools')}
            </Heading>
            <Heading scale="md" color="text">
            {isBounties ? t('Create a trust bounty to improve trust with your customers.') : t('Create a contract and start receiving bets on various events')}
            </Heading>
            <Heading scale="md" color="text">
            ...
            <Flex>
              <Button p="0" variant="text">
                <Text color="primary" onClick={isBounties ? onPresentTrustBounties : onPresentCreateGauge} bold fontSize="16px" mr="4px">
                  {isBounties ? t('Create an Bounty in ') : t('Create a betting contract')}{' '}
                </Text>
                <CurrencyInputPanel
                  showInput={false}
