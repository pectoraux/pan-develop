...
import { InjectedModalProps, Button, Flex, NotificationDot, useToast } from '@pancakeswap/uikit'
...
interface BuyModalProps extends InjectedModalProps {
  variant: "admin" | "user" | "delete" | "buy" | "init"
...
const BuyModal: React.FC<any> = ({ variant="user", sessionId, location="arp", pool, currency, onDismiss }) => {
  ...
  //           amount: parseInt(state.amountReceivable) * 100,
  //           currency: currency?.symbol?.slice(1,)?.toLowerCase(),
  //           account
  //         })
  //         console.log("data.err1==================>", data)
  //         if (!data.err) {
  //           toastSuccess(t('Successful transferred %amount% to the bank', { amount: data.amount }))
  //         } else {
  //           console.log("data.err==================>", data.err)
  //           // throw new Error('cannot transfer to bank') 
  //         }
  //       }
      default:
        break
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onConfirm: () => {
      ...
      if (stage === LockStage.CONFIRM_MINT) {
        ...
        console.log("CONFIRM_MINT===============>",[data?.tokenAddress, account, amount.toString(), state.identityTokenId, state.sessionId])
        return callWithGasPrice(rampContract, 'mint', [data?.tokenAddress, account, amount.toString(), state.identityTokenId, state.sessionId])
      ...
      if (stage === LockStage.CONFIRM_BURN) {
        ...
        return callWithGasPrice(rampContract, 'burn', [state.token, amount.toString(), state.identityTokenId])
      ...
      if (stage === LockStage.CONFIRM_UPDATE_ADMIN) {
        return callWithGasPrice(rampContract, 'updateAdmin', [
          state.owner,
          !!state.add,
