...
import { useERC20, useBILLContract, useBILLHelper } from 'hooks/useContract'
...
const BuyModal: React.FC<any> = ({ variant="user", pool, currency, onDismiss }) => {
  ...
  console.log("mcurrencyy===============>", currency, pool, billContract)
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    name: pool?.name,
    applicationLink: pool?.bill?.applicationLink,
    billDescription: pool?.bill?.billDescription,
    contactChannels: [],
    contacts: [],
    workspaces: [],
    countries: [],
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onApproveSuccess: async ({ receipt }) => {
      toastSuccess(
        t('Contract approved - you can now transfer tokens into this BILL!'),
    ...
    onConfirm: () => {
      if (stage === LockStage.CONFIRM_UPDATE_PROTOCOL) {
        ...
        return callWithGasPrice(billContract, 'updateProtocol', [
          account,
          [amountReceivable.toString(), state.periodReceivable, startReceivable.toString()],
      ...
      if (stage === LockStage.CONFIRM_UPDATE_PARAMETERS) {
        return callWithGasPrice(billContract, 'updateParameters', [
      ...
      if (stage === LockStage.CONFIRM_ADMIN_AUTOCHARGE) {
        return callWithGasPrice(billContract, 'autoCharge', [state.accounts, 0, 1])
        .catch((err) => console.log("CONFIRM_ADMIN_AUTOCHARGE===============>", err, billContract))
      }
      if (stage === LockStage.CONFIRM_UPDATE_DESCRIPTION) {
        return callWithGasPrice(billHelperContract, 'emitUpdateBillDescription', [
          pool.billAddress, 
      ...
      if (stage === LockStage.CONFIRM_WITHDRAW) {
        ...
        return callWithGasPrice(billContract, 'withdraw', [
          currency.address, 
          amount.toString()
        ])
      ...
      if (stage === LockStage.CONFIRM_DELETE_PROTOCOL) {
        return callWithGasPrice(billContract, 'deleteProtocol', [currAccount])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_DELETE) {
        return callWithGasPrice(billHelperContract, 'deleteBill', [pool?.billAddress])
        .catch((err) => console.log("CONFIRM_DELETE_PROTOCOL===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_BOUNTY_ID) {
        return callWithGasPrice(billContract, 'updateBounty', [state.bountyId])
          .catch((err) => console.log("CONFIRM_UPDATE_BOUNTY===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_OWNER) {
        return callWithGasPrice(billContract, 'updateOwner', [account, state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_OWNER===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_TOKEN_ID) {
        return callWithGasPrice(billContract, 'updateTokenId', [state.tokenId])
        .catch((err) => console.log("CONFIRM_UPDATE_TOKEN_ID===============>", err))
      }
      if (stage === LockStage.CONFIRM_UPDATE_AUTOCHARGE) {
        return callWithGasPrice(billContract, 'updateAutoCharge', [!!state.autoCharge])
        .catch((err) => console.log("CONFIRM_UPDATE_AUTOCHARGE===============>", err))
      }
      if (stage === LockStage.CONFIRM_VOTE) {
        return callWithGasPrice(billHelperContract, 'vote', [
          pool.billAddress,
          state.profileId,
          !!state.like
        ])
