      ...
      <Flex flexDirection='column'>
        <Flex flexDirection='row'>
          ...
          <Balance
            ...
            color={Number(pool.bill?.likes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.bill?.likes) ? pool.bill?.likes : 0}
          />
        </Flex>
        <Flex flexDirection='row' position='relative' bottom='5px'>
          ...
          <Balance
            bold={!isMobile}
            fontSize={isMobile ? '14px' : '16px'}
            color={Number(pool.bill?.dislikes) ? 'primary' : 'textDisabled'}
            decimals={0}
            value={Number(pool.bill?.dislikes) ? pool.bill?.dislikes : 0}
          />
        </Flex>
      </Flex>
    </StyledCell>
