...
const PoolStatsInfo: React.FC<any> = ({
  ...
  const tokenAddress = pool?.earningToken?.address || ''
  ...
  return (
    <Flex flexDirection='column' maxHeight='200px' overflow='auto'>
      ...
      <Flex flex="1" flexDirection="column" alignSelf="flex-center">
        ...
        {Number(pool?.profileId) ?
        <Text color="primary" fontSize="14px">
          {t("Admin Profile Id")} {`->`} {pool?.profileId}
        </Text>:null}
      ...
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
        <LinkExternal href={`/info/token/${pool?.billAddress}`} bold={false} small>
          {t('See Bill Contract')}
        </LinkExternal>
      </Flex>
      <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
