...
import { useGetCollections, useGetNftActivityFilters } from 'state/cancan/hooks'
...
export const ListCollectionFilter: React.FC = () => {
  ...
  const filteredCollections = (
    query && query.length > 1
      ? Object.values(collections).filter((item: any) => item.name.toLowerCase().indexOf(query.toLowerCase()) !== -1)
  ...
  const handleItemClick = (evt:any, collection: any) => {
    if (evt.target.checked) {
      // dispatch(
        addActivityCollectionFilters({ address: collection.address.toLowerCase(), collection })
      // )
    } else {
      // dispatch(
        removeActivityCollectionFilters({ address: collection.address.toLowerCase(), collection })
      // )
    }
  }
