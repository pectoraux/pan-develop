...
const CreateProposal = () => {
  ...
  const handleCreatePitch = useCallback(async () => {
    ...
    console.log("createClaim===============>", acceleratorContract, [["","","",state.original,state.thumbnail],state.name,state.body])
      try {
  ...
  return (
    <Container py="40px">
      ...
      <Box mb="48px">
        <Breadcrumbs>
          <Link href="/accelerator">{t('Accelerator')}</Link>
          <Link href="/accelerator/voting">{t('Voting')}</Link>
