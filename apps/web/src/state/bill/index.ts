...
import { fetchBill } from 'state/bills/fetchPools'
...
export const fetchBillAsync = (billAddress) => async (dispatch) => {
  try {
    console.log("fetchBusinesses1================>", billAddress)
    const bill = await fetchBill(billAddress)
    console.log("fetchBusinesses================>", bill, billAddress)
    dispatch(setBillPublicData([bill] || []))
  } catch (error) {
    console.error('[Pools Action] error when getting staking limits', error)
  }
}

export const fetchBillUserDataAsync = createAsyncThunk<
...
export const PoolsSlice = createSlice({
  ...
  initialState,
  reducers: {
    setBillPublicData: (state, action) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchBillUserDataAsync.fulfilled,
      (
        state,
    ...
    builder.addCase(fetchBillUserDataAsync.rejected, (state, action) => {
