...
import { fetchArp } from 'state/arps/fetchPools'
...
export const fetchArpAsync = (arpAddress) => async (dispatch) => {
  try {
    console.log("fetchBusinesses1================>", arpAddress)
    const arp = await fetchArp(arpAddress)
    console.log("fetchBusinesses================>", arp, arpAddress)
    dispatch(setArpPublicData([arp] || []))
  } catch (error) {
    console.error('[Pools Action] error when getting staking limits', error)
  }
}

export const fetchArpUserDataAsync = createAsyncThunk<
...
export const PoolsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchArpUserDataAsync.fulfilled,
      (
        state,
    ...
    builder.addCase(fetchArpUserDataAsync.rejected, (state, action) => {
