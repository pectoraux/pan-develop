...
export default createReducer(initialState, (builder) =>
  builder
    ...
    .addCase(addVeToken, (state, { payload: { workspace, tokenIDs } }) => {
      if (!state.nfts) {
        state.nfts = {}
      }
      state.nfts[workspace] = state.nfts[workspace] || []
      state.veWorkspaces = state.veWorkspaces || []
      if(!(state.nfts[workspace] && state.nfts[workspace].length)) {
        state.veWorkspaces.push(workspace)
      }
      state.nfts[workspace] = tokenIDs
    })
