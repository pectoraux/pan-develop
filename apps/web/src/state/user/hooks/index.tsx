...
import { GAS_PRICE_GWEI, State } from '../../types'
...
export const useWatchlistTokens = (): any => {
  const dispatch = useAppDispatch()
  const savedTokens = useSelector((state: any) => state.user.watchlistTokens) ?? []
...
export const useGetUserVes = () => {
  const ves = useSelector((state: State) => state.user.nfts || [])
  return ves
}
