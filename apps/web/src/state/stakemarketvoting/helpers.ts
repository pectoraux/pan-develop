...
import { SNAPSHOT_API, GRAPH_API_SM_VOTER } from 'config/constants/endpoints'
import request, { gql } from 'graphql-request'
import { Proposal, ProposalState, Vote, VoteWhere } from 'state/types'
import _chunk from 'lodash/chunk'
import { getValuepoolVoterContract, getVeContract } from '../../utils/contractHelpers'
...
export const getVotingPower = async (veAddress, tokenId) => {
  const veContract = getVeContract(veAddress)
  try {
    const [
      decimals,
      balance,
      ...
      veContract.percentiles(tokenId),
    ])
