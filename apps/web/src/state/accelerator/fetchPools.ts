...
export const fetchAccelerator = async () => {
  ...
  const businesses =  await Promise.all(
    gauges.map(async (gauge) => {
      ...
      const totalWeightRaw = await acceleratorVoterContract.totalWeight(gauge.ve)
      const totalWeight = Number(totalWeightRaw) ? new BigNumber(totalWeightRaw._hex) : BIG_ONE
      const gaugeWeight = await acceleratorVoterContract.weights(gauge.id, gauge.owner)
      const claimable = await acceleratorVoterContract.claimable(gauge.gauge)
...
export const fetchPoolsWeights = async (businesses) => {
  ...
  // const weights = await multicall(acceleratorVoterABI, calls)
  return null
}