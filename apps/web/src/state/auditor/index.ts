...
import { fetchAuditor } from 'state/auditors/fetchPools'
...
export const fetchAuditorAsync = (auditorAddress) => async (dispatch) => {
  try {
    console.log("fetchBusinesses1================>", auditorAddress)
    const auditor = await fetchAuditor(auditorAddress)
    console.log("fetchBusinesses================>", auditor, auditorAddress)
    dispatch(setAuditorPublicData([auditor] || []))
  } catch (error) {
    console.error('[Pools Action] error when getting staking limits', error)
  }
}

export const fetchAuditorUserDataAsync = createAsyncThunk<
...
export const PoolsSlice = createSlice({
  ...
  initialState,
  reducers: {
    setAuditorPublicData: (state, action) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchAuditorUserDataAsync.fulfilled,
      (
        state,
    ...
    builder.addCase(fetchAuditorUserDataAsync.rejected, (state, action) => {
