...
import { fetchCard } from 'state/cards/fetchPools'
...
export const fetchCardAsync = (cardAddress) => async (dispatch) => {
  try {
    console.log("fetchBusinesses1================>", cardAddress)
    const card = await fetchCard(cardAddress)
    console.log("fetchBusinesses================>", card, cardAddress)
    dispatch(setCardPublicData([card] || []))
  } catch (error) {
    console.error('[Pools Action] error when getting staking limits', error)
  }
}

export const fetchCardUserDataAsync = createAsyncThunk<
...
export const PoolsSlice = createSlice({
  ...
  initialState,
  reducers: {
    setCardPublicData: (state, action) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchCardUserDataAsync.fulfilled,
      (
        state,
    ...
    builder.addCase(fetchCardUserDataAsync.rejected, (state, action) => {
